...
module.exports = async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      avatar && (updateUser.avatar = avatar)
      // 密码加密
      if (password) {
        ...
        // 将用户添加到数据库中
        await users.updateOne({ account }, { ...updateUser, password: bcryptPassword })
      } else {
        // 将用户添加到数据库中
        await users.updateOne({ account }, { ...updateUser, password: user.password })
      }
      // 修改所有的外出申请信息
      const gooutList = await goout.findOne({ account })
      gooutList && await goout.updateMany({ account }, { $set: { name, phone, address: Number(address) } })
      const checkList = await check.findOne({ account })
      checkList && await check.updateMany({ account }, { $set: { name, phone, address: Number(address) } })
      // 修改所有的文章信息
      const articleList = await article.findOne({ account })
